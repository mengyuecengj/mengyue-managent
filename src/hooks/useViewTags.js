...
export function useTags() {
    ...
    const context = {
         ...
         */
        openPlaceholder(tag, clientX, clientY) {
    ...
    async function openContextMenu(tag, e) {
        // 阻止默认浏览器菜单与冒泡
        e.preventDefault();
        e.stopPropagation();
        // 先显示占位菜单，给用户即时反馈
    ...
    // 判断标签是否为当前激活
    const isActive = (tag) => getNormalPath(tag.path) === getNormalPath(route.path);
    // 注册父组件传入的 scroll 组件 ref（支持 Ref 或 直接实例）
    function registerScrollRef(refEl) {
    ...
    async function navigaToLastView(views) {
    ...
    async function onClose(tag) {
        if (!tag || loading.value)
            return;
        if (tag.meta?.affix) {
    ...
    // 中键(鼠标滚轮)关闭
    function onMiddleClick(tag) {
        if (!tag.meta?.affix)
            onClose(tag);
    }
    // 刷新页面（并处理 iframe link 的清理）
    async function onRefresh(tag) {
        if (!tag)
            return;
        context.close();
        ...
        try {
            ...
            if (route.meta?.link) {
                await store.deleteIframeView(route);
            }
    ...
    // 打开右键菜单（对外接口供模板事件调用）
    function openContextMenuHandler(tag, e) {
