...
export const useDashboardStore = defineStore('dashboard', () => {
    ...
    const createChartBlock = (item, componentConfig, defaultName) => {
    ...
    const createDecorationBlock = (item, componentConfig, defaultName) => {
        return {
            rendererType: 'border',
            config: item.config || {},
    ...
    const createTextBlock = (item, componentConfig, defaultName) => {
        return {
            rendererType: 'text',
            config: {
                text: '这是一段文本内容',
    ...
    const createMapBlock = (item, componentConfig, defaultName) => {
        const config = cloneDeep(componentConfig);
    ...
    const addBlock = (value, componentConfig, x = 200, y = 200, name) => {
    ...
    const saveCurrentDashboard = () => {
        localStorage.setItem(`dashboard_${currentId.value}`, JSON.stringify(dashboards.value[currentId.value]));
        modal.msgSuccess('保存成功');
    };
    const restoreDashboard = (id) => {
