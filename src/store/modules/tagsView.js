...
export const useTagsViewStore = defineStore('tags-view', {
    ...
    actions: {
         ...
         */
        addView(view) {
            if (this.visitedViews.some(v => v.path === view.path && v.fullPath === view.fullPath))
                return;
            const newView = {
                ...view,
        ...
        async delOthersViews(view) {
            ...
            this.cachedViews = this.cachedViews.filter(name => this.visitedViews.some(v => typeof v.name === 'string' && v.name === name));
            // iframeViews 重算
            this.iframeViews = this.iframeViews.filter(iv => this.visitedViews.some(v => v.fullPath === iv.fullPath));
            saveState(this.$state);
        },
        // 关闭左侧：保留当前及其右侧（及 affix）
        async closeLeft(view) {
            const idx = this.visitedViews.findIndex(v => v.fullPath === view.fullPath);
            if (idx === -1)
