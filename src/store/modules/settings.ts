...
import { LayoutSetting } from '@/types/store/setting'
...
// 计算两个颜色的对比度比率（WCAG标准）
const getContrastRatio = (bgHex: string, textHex: string): number => {
...
const useSettingStore = defineStore('settings', {
  ...
  getters: {
    shouldUseWhiteText: () => (bgHex: string): boolean => {
  ...
  actions: {
    ...
    changeSetting(data: LayoutSetting) {
