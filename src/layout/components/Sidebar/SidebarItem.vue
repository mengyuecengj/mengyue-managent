  ...
  <div v-if="!item.hidden">
    <template
      ...
      <MYMenu-item v-if="onlyOneChild.meta" :index="resolvePath(onlyOneChild.path ?? '')"
        ...
        <span class="menu-title" :title="hasTitle(onlyOneChild.meta?.title ?? '')">{{ t(onlyOneChild.meta?.title ?? '')
        }}</span> </MYMenu-item>
    </template>

    <MYMenu-submenu v-else ref="subMenu" :index="resolvePath(item.path ?? '')" :class="getMenuItemClass(item)">
      <template #icon>
...
function getMenuItemClass(route: Partial<RouteItem>): string {
  ...
  // 如果是首页，添加特殊 class
  if (route.path === '/index' || route.meta?.title === 'menu.index') {
